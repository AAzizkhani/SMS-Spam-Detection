...
axes[0, 1].hist([df[df['label']=='ham']['msg_length'],
                  df[df['label']=='spam']['msg_length']],
                 bins=30, label=['Ham', 'Spam'], color=['#2ecc71', '#e74c3c'], alpha=0.7)
axes[0, 1].set_title('Message Length Distribution')
axes[0, 1].set_xlabel('Length')
axes[0, 1].legend()

vocab_data = {'Original': len(original_vocab),
...
axes[1, 1].boxplot([df['token_count'], df['stem_count'], df['lemma_count']],
...
plt.show()
print("\nPlot saved as 'nlp_analysis.png'")


df_final = df[['label', 'message', 'cleaned_text',
               'tokens_no_stopwords', 'stemmed_text', 'lemmatized_text']]

df_final.to_csv('sms_spam_preprocessed.csv', index=False)
print("\nFinal dataset saved as 'sms_spam_preprocessed.csv'")
